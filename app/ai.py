...
_NARRATIVE_VERB_RE = re.compile(
    # Между именем и глаголом допускаем ОДНО слово-наречие («Анна успешно прошла...») ИЛИ
    # тире-разделитель, частый в буллет-списках историй («🔹 Мария - поступила на...»).
    r"\b([А-ЯЁ][а-яё]{2,})\s+(?:[а-яё]+\s+|[-–—]\s+)?"
