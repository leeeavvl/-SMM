...
- ИЗБЕГАЙ универсальных списков «навыков будущего» / «качеств успеха», которые подошли бы
  ЛЮБОЙ профессии без изменений (критическое мышление, коммуникация, цифровая грамотность,
  эмпатия, обучаемость — такой список одинаково годится и для юриста, и для маркетолога, и
  для врача). Если тема про юристов — КАЖДЫЙ пункт должен быть узнаваемо юридическим:
  называй конкретный инструмент, технологию, ситуацию из практики или цифру, а не абстрактное
  качество без примера. Плохо: «Цифровая грамотность — технологии станут частью работы».
  Хорошо: «Работа с AI-инструментами для анализа судебной практики и подготовки договоров —
  например, автоматическая проверка контракта на риски за минуты вместо часов вручную».
  Не заканчивай список пунктом-водой, который просто пересказывает заголовок другими
  словами без новой информации («мир меняется, поэтому важно постоянно учиться»).
...
4. Финал — конкретный призыв к действию, а не шаблонное «надеюсь, было полезно» или
   ...
   финальный призыв/вопрос. Финальный призыв ОБЯЗАТЕЛЕН всегда, без исключений — пост
   НЕ должен просто обрываться на последнем пункте списка/аргументе без какого-либо
   завершения. Даже если пост чисто информационный без явного «продающего» повода —
   заверши мягким вовлечением (вопрос, приглашение поделиться опытом), а не тишиной.
...
7. Пост — ОДНО связное целое с ОДНОЙ линией мысли от хука до финала, а НЕ несколько
   ...
   Похожая ошибка — пост из 5+ отдельных подзаголовков-вопросов («Какие ошибки чаще
   всего встречаются?», «Почему важна конкретика?», «Как выделиться среди кандидатов?»,
   «Какие показатели помогут?»), где несколько из них по факту раскрывают ОДНУ и ту же
   мысль другими словами (например три раздела подряд об одном — «пишите цифры и
   результаты, а не общие фразы»). Это не склеенные мини-посты с CTA внутри, но то же
   ощущение случайного набора идей вместо одной выстроенной мысли. Перед финальным
   ответом проверь: если два подзаголовка можно объединить без потери смысла — объедини
   их, не растягивай пост на лишние разделы ради объёма.
...
_BANNED_HOOK_RE = re.compile(
    ...
    # «Сегодня (мы) поговорим/расскажем/разберём о том, как...» — второй самый частый
    # запрещённый шаблон открытия после «представь» (дословный пересказ темы вместо хука,
    # см. СТРУКТУРА ПОСТА п.1), реальный промах на живой генерации.
...
_META_LABEL_RE = re.compile(
    ...
    # 1. Метка + ДВОЕОБЯЗАТЕЛЬНОЕ двоеточие — срезаем всегда, даже если после двоеточия
    #    сразу идёт текст на той же строке («Призыв к действию: Напишите...» -> «Напишите...»).
    # 2. Метка БЕЗ двоеточия — отдельная строка-заголовок («🧩 Призыв к действию  \n...»,
    #    реальный промах) — срезаем, ТОЛЬКО если до конца строки больше ничего нет, иначе
    #    это легитимное предложение («Призыв к действию должен быть чётким») и не трогаем.
    r"^[^\w\n]{0,10}(?:(?:призыв\s+к\s+действию|хук|тезис)\s*:[ \t]*\n?"
    r"|(?:призыв\s+к\s+действию|хук|тезис)[ \t]*(?:\n|$))",
...
# Одиночная звёздочка — markdown-курсив («*коммерческое предложение*») — реальный
# промах на живой генерации: старый regex ловил только **жирный** (двойная звёздочка).
...
_BANNED_HOOK_PREFIX_STRIP_RE = re.compile(
    # Именованные группы lead1/lead2 — чтобы функция ниже могла найти «ведущие» эмодзи
    # независимо от того, какая из двух альтернатив сработала (у них разная нумерация
    # групп из-за разного числа плейсхолдеров внутри).
    r"^(?P<lead1>[\W\d]{0,15})(представь(те)?|вообрази(те)?)\b(\s*себе\b)?[,:]?\s*(что\b\s*)?"
    r"|^(?P<lead2>[\W\d]{0,15})сегодня\s+(мы\s+)?(поговорим|расскажем|разберём|разберем|обсудим)\b"
    r"\s*(о том\s*)?,?\s*(как\s+)?",
    re.IGNORECASE | re.UNICODE,
)
...
def _strip_banned_hook_prefix(body: str) -> str:
    ...
    if not match:
        return body
    leading = match.group("lead1") or match.group("lead2") or ""
...
_CTA_TRIGGER_RE = re.compile(
    # «Напиши[те] [.{0,30} в] комментариях» — промежуточная часть НЕОБЯЗАТЕЛЬНА, чтобы ловить
    # и прямое «Напиши в комментарии», и «Напиши слово «резюме» в комментариях» (баг прошлого
    # фикса: требование ДВУХ отдельных \s+ ломало прямой вариант без промежуточных слов).
    r"оставьте заявку|оставить заявку|напиши[а-я]*(?:\s+\S+){0,4}?\s+в\s+коммент\w*|"
...
_IMAGINE_SCENARIO_RE = re.compile(
    # «представь(те)»/«вообрази(те)» — форма 2-го лица почти всегда именно запрещённый приём
    # «вообрази/представь себе...», ловим ЛЮБОЕ продолжение после них (не только «что»/
    # «ситуация», реальный промах: «Представь, КАК твои руки потеют...»). \b...\b на границах
    # слова специально отсекает «представить»/«представил»/«представленный» — это легитимные
    # деловые формы («представить документы в суд»), а не запрещённый приём с воображением.
    # «представим»/«вообразим» (1-е лицо мн.ч.) — не трогаем без «что»/«ситуацию» рядом:
    # «мы представим результаты на следующей неделе» — обычное будущее время, не воображение.
    r"\bпредставь(те)?\b|\bвообрази(те)?\b"
    r"|\bпредставим\b\s*(себе\s+)?(,\s*)?(что\b|как\b|ситуаци\w*)"
    r"|\bвообразим\b\s*(себе\s+)?(ситуаци\w*)?",
    re.IGNORECASE,
)
...
_FABRICATED_PERSON_RE = re.compile(
    ...
    r"\b(?i:студент\w*|выпускник\w*|выпускниц\w*|резидент\w*|стажёр\w*|стажер\w*|клиент\w*|юрист\w*)\s+(?P<name1>[А-ЯЁ][а-яё]{2,})\b"
    # до двух слов-определений между запятой и словом-ролью: «Алексей, наш вчерашний выпускник, ...»
    r"|\b(?P<name2>[А-ЯЁ][а-яё]{2,}),\s*(?:[а-яё]+\s+){0,2}(?i:студент\w*|выпускник\w*|выпускниц\w*|резидент\w*|стажёр\w*|стажер\w*|клиент\w*|юрист\w*)\b",
    re.UNICODE,
)
...
_NARRATIVE_VERB_RE = re.compile(
    # Между именем и глаголом допускаем: ОДНО слово-наречие («Анна успешно прошла...»),
    # тире-разделитель («Мария - поступила на...»), ИЛИ вставную характеристику в запятых
    # («Анастасия, ранее столкнувшаяся с отказами, изменила дизайн...»).
    r"\b([А-ЯЁ][а-яё]{2,})(?:\s+(?:[а-яё]+\s+|[-–—]\s+)?|,\s*[^,\n]{1,60},\s*)"
    # «стал[а-я]*»/«прошл[а-я]*» с открытым суффиксом ловили посторонние слова
    # («сталкиваются», «прошлое») — заменены на точный список словоформ.
    r"(нашл[а-я]*|получил[а-я]*|поступил[а-я]*|устроил[а-я]*|прошл[аи]|прошло|заверш[а-я]*"
    r"|начал[а-я]*|решил[а-я]*|поделил[а-я]*|рассказал[а-я]*|справил[а-я]*|добил[а-я]*с[а-я]*"
    r"|стал(?:[аи])?|стало|отправил[а-я]*|написал[а-я]*|ответил[а-я]*|выбрал[а-я]*|изменил[а-я]*"
    r"|перестроил[а-я]*|сменил[а-я]*|прошёл|прошла|окончил[а-я]*|показал[а-я]*|открыл[а-я]*"
    r"|освоил[а-я]*|преодолел[а-я]*|достиг[а-я]*|доказал[а-я]*|заняла?|проявил[а-я]*)\b",
    re.UNICODE,
)
_NAME_STOPWORDS = {
